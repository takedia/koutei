...
## auth.json — アプリのログイン & 管理者パスワード

```json
{
  "passwordHash": "b34c887aea7910ac49db53cf2745cd1187e38fb51f70c097abe4d0124e969d5b",
  "adminPasswordHash": "6051fc84a7a0d74c225fb18a496b09952da5642e60723ecae543298edd7d82d6"
}
```

- `passwordHash` — アプリ起動時にユーザーが入力するログインパスワードの SHA-256 hex（小文字）
- `adminPasswordHash` — 設定画面の「ハッシュ生成ツール」を開くための管理者パスワードの SHA-256 hex
  - これがあるとアプリにログインできる人でもパスワード変更ツールに触れない（あなただけが知る合言葉）
...
## recipients.json — メール送信の共有宛先

```json
{
  "presets": [
    { "ラベル": "会社", "メアド": "company@example.com" },
    { "ラベル": "下請A", "メアド": "subcontractor@example.com" }
  ]
}
```

全端末・全ユーザーで共通の宛先リスト。アプリの設定画面の「管理者メニュー」から
編集 → 「JSON コピー」→ ここに貼り付けてコミット、で反映できます（1〜5 分）。

## ログインパスワードを無効化（誰でも起動できるように）

`passwordHash` を空文字にしてコミットすれば OK です:
...
→ アプリ起動時のパスワード入力画面がスキップされ、即ホーム画面が開く。
管理者メニューだけは引き続き `adminPasswordHash` で守られる。
